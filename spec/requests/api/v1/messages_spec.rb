...
RSpec.describe 'api/v1/messages', type: :request do
  ...
  let(:conversation1) { create(:conversation, user1_id: user1.id, user2_id: user2.id) }
  ...
  context 'with valid authorization token' do
    it 'creates a new message and renders a successful response' do
      post api_v1_messages_url, params: valid_attributes, headers: valid_token, as: :json
      expect(response).to have_http_status(:ok)
    end

    it 'returns an error for missing param' do
      post api_v1_messages_url, params: invalid_attributes, headers: valid_token, as: :json
  ...
  context 'without valid authorization token' do
    it 'returns an error for invalid or unexisting token' do
      post api_v1_messages_url, params: valid_attributes, headers: {}, as: :json
